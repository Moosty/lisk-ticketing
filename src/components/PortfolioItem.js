...
export const PortfolioItem = withReducer("portfolioItem", reducer)(({ticketType, eventId, eventDate, eventTime,type, title, day, month, time, artist, location, keyEvent}) => {
  ...
  const eventX = events.find(event => event.address === keyEvent);
  const eventData = eventX.asset.eventData;

  const ticketData = eventX.asset.ticketData.types.find(type => type.id === ticketType );
